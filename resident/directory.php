<?php
// resident/directory.php - Estate Community Member & Neighbor Directory
...
// Query all active estate residents across all zones
$sql = "SELECT 
    r.id as resident_id,
    r.custom_id,
    r.flat_id,
...
// Fetch Zones & Streets for filtering
...
<style>
/* Resident Portal Theme Overrides */
.directory-hero {
...
<div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3 mb-4">
    <div>
        <div class="header-breadcrumbs">
            ...
            <span class="active">Community Directory</span>
        </div>
        <h2 class="h4 font-bold text-slate-800 m-0">Community Member Directory</h2>
        <p class="text-secondary small mb-0">Connect with fellow residents, neighbors, and estate families across the entire community.</p>
    </div>
    <div class="d-flex flex-wrap align-items-center gap-2">
        ...
        <button class="btn btn-sm btn-primary" id="toggleAllBtn" onclick="toggleAllDependants()">
...
<div class="directory-hero d-flex flex-wrap align-items-center justify-content-between gap-3">
    <div class="d-flex align-items-center gap-3">
        <div style="width: 48px; height: 48px; border-radius: 12px; background: rgba(37, 99, 235, 0.15); color: #2563eb; display: flex; align-items: center; justify-content: center; font-size: 1.4rem;">
            <i class="fa-solid fa-address-book"></i>
        </div>
        <div>
            <h4 class="fw-bold text-slate-900 m-0" style="font-size: 1.15rem;">Estate Resident Phonebook</h4>
            <span class="text-secondary small">Community contact directory with primary households and family dependants.</span>
        </div>
    </div>
    <div class="d-flex flex-wrap align-items-center gap-2">
        <div class="directory-stat-badge">
            <i class="fa-solid fa-users text-primary"></i>
            <span><?php echo number_format($total_members); ?> Estate Members</span>
        </div>
        <div class="directory-stat-badge">
            <i class="fa-solid fa-user-tie text-success"></i>
            <span><?php echo number_format($total_heads); ?> Household Heads</span>
        </div>
        <div class="directory-stat-badge">
            <i class="fa-solid fa-people-roof text-info"></i>
            <span><?php echo number_format($total_dependents); ?> Family Dependants</span>
        </div>
        <div class="directory-stat-badge">
            <i class="fa-solid fa-house-chimney text-secondary"></i>
            <span><?php echo number_format($total_households); ?> Residences</span>
        </div>
    </div>
</div>
...
<div class="directory-toolbar">
    <div class="d-flex flex-column flex-lg-row align-items-stretch align-items-lg-center justify-content-between gap-3">
        <div class="search-input-wrap">
            ...
            <input type="text" id="dirSearch" placeholder="Search neighbor name, phone, email, unit, or street..." onkeyup="filterDirectory()">
        </div>
        ...
        <div class="d-flex flex-wrap align-items-center gap-2">
            ...
            <select id="depFilter" class="filter-select" onchange="filterDirectory()">
                ...
                <option value="has_dep">With Family Members</option>
                <option value="solo">Single Occupant Only</option>
            </select>
...
<div class="d-flex justify-content-between align-items-center mb-3 text-secondary small px-1">
    <span>Showing <strong id="visibleCount" class="text-slate-900"><?php echo count($households); ?></strong> residences</span>
</div>
...
<div id="dirGridView" class="directory-grid">
    ...
    <div class="dir-card" 
        ...
        <div class="dir-card-header">
            ...
            <?php else: ?>
                <div class="dir-avatar" style="background: linear-gradient(135deg, #1e40af 0%, #3b82f6 100%);"><?php echo $initials; ?></div>
            <?php endif; ?>
            ...
            <div class="dir-info">
                ...
                <div class="d-flex flex-wrap align-items-center gap-1">
                    ...
                    <?php if (!empty($addr['zone_name'])): ?>
                        <span class="badge" style="font-size: 0.68rem; background: rgba(59,130,246,0.1); color: #2563eb;"><?php echo htmlspecialchars($addr['zone_name']); ?></span>
                    <?php endif; ?>
                </div>
            ...
            <div class="contact-row">
                ...
                ?>
                    <div class="contact-actions">
                        <a href="tel:<?php echo htmlspecialchars($p['phone']); ?>" class="quick-action-btn call-btn" title="Call Neighbor">
                            <i class="fa-solid fa-phone"></i>
                        </a>
                        <a href="https://wa.me/<?php echo $clean_phone; ?>" target="_blank" class="quick-action-btn wa-btn" title="WhatsApp">
        ...
        <div class="dir-dependants-wrapper">
                ...
                <button type="button" class="dependants-toggle-bar" onclick="toggleDependants('<?php echo $f_key; ?>', this)">
                    <div class="d-flex align-items-center gap-2">
                        <span class="dep-count-badge">
                            <i class="fa-solid fa-people-roof"></i> Family Members (<?php echo $dep_count; ?>)
                        </span>
                    ...
                    ?>
                        <div class="dependant-item-card">
                            <div class="dependant-header">
                                ...
                                <?php else: ?>
                                    <div class="dir-avatar-dependent" style="background: linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%);"><?php echo $d_initials; ?></div>
                                <?php endif; ?>
            ...
            <?php else: ?>
                <div class="dependants-toggle-bar no-dependants">
                    <span class="text-muted small"><i class="fa-regular fa-circle-check me-1"></i> Single Occupant</span>
                </div>
            <?php endif; ?>
        </div>
    </div>
        ...
        <table class="dir-table">
            ...
            <tbody>
                ...
                <tr class="primary-row" 
                    ...
                    <td>
                        <div class="d-flex align-items-center gap-2">
                            <div class="dir-avatar" style="width: 38px; height: 38px; font-size: 0.85rem; border-radius: 8px; background: linear-gradient(135deg, #1e40af 0%, #3b82f6 100%);">
                                <?php echo $initials; ?>
                            </div>
                            <div>
                                <div class="fw-bold text-slate-900"><?php echo htmlspecialchars($p['name']); ?></div>
                                <span class="badge bg-slate-100 text-secondary" style="font-size: 0.7rem;"><?php echo ucfirst($p['relationship'] ?? 'Resident'); ?></span>
                            </div>
                        </div>
                    </td>
                    <td>
                    ...
                    <td>
                        <?php if ($dep_count > 0): ?>
                            <span class="dep-count-badge" style="cursor: pointer;" onclick="toggleTableNestedRow('<?php echo $f_key; ?>')">
                                <i class="fa-solid fa-people-roof"></i> <?php echo $dep_count; ?> Members
                            </span>
                        <?php else: ?>
                            ...
                            <div class="nested-dependants-inner">
                                <div class="d-flex align-items-center gap-2 mb-2">
                                    <span class="fw-bold text-slate-800 small text-uppercase" style="letter-spacing: 0.04em;">
                                        <i class="fa-solid fa-people-roof text-primary me-1"></i> Family Members Registered to <?php echo htmlspecialchars($unit_str); ?> (<?php echo $dep_count; ?>)
                                    </span>
                                </div>
                                    ...
                                    <tbody>
                                        <?php foreach ($deps as $d): ?>
                                            <tr>
                                                ...
                                                <td><span class="dep-rel-tag"><?php echo htmlspecialchars(ucfirst($d['relationship'] ?? 'Family Member')); ?></span></td>
...
<div id="dirEmptyState" class="text-center py-5" style="display: none;">
    ...
    <h5 class="fw-bold text-slate-800 mb-1">No Matching Community Members</h5>
    <p class="text-secondary small mb-3">No neighbors or family members matched your search filters.</p>
...
function exportDirectoryCSV() {
    ...
    link.setAttribute("download", "Community_Directory_" + new Date().toISOString().slice(0, 10) + ".csv");
