<?php
// zone/directory.php - Estate-Wide Member & Contact Directory for Zonal Portal
...
// Full estate-wide access across all zones as requested
$sql = "SELECT 
    r.id as resident_id,
    r.custom_id,
    r.flat_id,
...
ORDER BY (CASE WHEN z.id = $current_zone_id THEN 0 ELSE 1 END), z.name ASC, s.name ASC, b.name ASC, f.number ASC, (CASE WHEN r.type = 'head' THEN 0 ELSE 1 END), r.id ASC";
...
if ($residents_res) {
    while ($row = $residents_res->fetch_assoc()) {
        $total_members++;
        if (intval($row['zone_id'] ?? 0) === $current_zone_id) {
            $my_zone_count++;
        }
...
// Fetch all Zones & Streets across estate for filtering
...
<style>
/* Zonal Portal Theme Overrides for Directory */
.directory-hero {
...
<div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3 mb-4">
    <div>
        <div class="header-breadcrumbs">
            <span><?php echo htmlspecialchars($zone_name_display); ?></span>
        ...
        <p class="page-subtitle mb-0">Estate-wide resident phonebook with household heads and nested family dependants.</p>
    </div>
    <div class="d-flex flex-wrap align-items-center gap-2">
        ...
        <button class="btn btn-sm btn-outline-purple" id="toggleAllBtn" style="color: #7e22ce; border: 1px solid rgba(168,85,247,0.4);" onclick="toggleAllDependants()">
...
<div class="directory-hero d-flex flex-wrap align-items-center justify-content-between gap-3">
    <div class="d-flex align-items-center gap-3">
        <div style="width: 48px; height: 48px; border-radius: 12px; background: rgba(168, 85, 247, 0.15); color: #9333ea; display: flex; align-items: center; justify-content: center; font-size: 1.4rem;">
            <i class="fa-solid fa-address-book"></i>
        </div>
        <div>
            <h4 class="fw-bold text-slate-900 m-0" style="font-size: 1.15rem;">Estate Occupant Directory</h4>
            <span class="text-secondary small">Comprehensive directory with cross-zone visibility.</span>
        </div>
    </div>
    <div class="d-flex flex-wrap align-items-center gap-2">
        <div class="directory-stat-badge">
            <i class="fa-solid fa-users text-purple" style="color: #9333ea;"></i>
            <span><?php echo number_format($total_members); ?> Estate Members</span>
        </div>
        <div class="directory-stat-badge">
            <i class="fa-solid fa-layer-group text-primary"></i>
            <span><?php echo number_format($my_zone_count); ?> In <?php echo htmlspecialchars($zone_name_display); ?></span>
        </div>
        <div class="directory-stat-badge">
...
<div class="directory-toolbar">
    <div class="d-flex flex-column flex-lg-row align-items-stretch align-items-lg-center justify-content-between gap-3">
        ...
        <!-- Filter Selects -->
        <div class="d-flex flex-wrap align-items-center gap-2">
            <!-- Zone Filter (Unrestricted) -->
            <select id="zoneFilter" class="filter-select" onchange="filterDirectory()">
                <option value="">All Zones (Full Estate)</option>
                <?php if ($zones_list): ?>
                    <?php while ($z = $zones_list->fetch_assoc()): ?>
                        <option value="<?php echo htmlspecialchars($z['name']); ?>">
                            <?php echo htmlspecialchars($z['name']); ?> <?php echo ($z['id'] == $current_zone_id) ? '(My Zone)' : ''; ?>
                        </option>
                    <?php endwhile; ?>
                <?php endif; ?>
            </select>
...
<div id="dirGridView" class="directory-grid">
    ...
    <div class="dir-card" 
        ...
        <div class="dir-card-header">
            ...
            <?php else: ?>
                <div class="dir-avatar" style="background: linear-gradient(135deg, #581c87 0%, #7e22ce 100%);"><?php echo $initials; ?></div>
            <?php endif; ?>
            ...
            <div class="dir-info">
                ...
                <div class="d-flex flex-wrap align-items-center gap-1">
                    ...
                    <?php if (!empty($addr['zone_name'])): ?>
                        <span class="badge" style="font-size: 0.68rem; background: rgba(168,85,247,0.12); color: #7e22ce;"><?php echo htmlspecialchars($addr['zone_name']); ?></span>
                    <?php endif; ?>
                </div>
        ...
        <div class="dir-contact-body">
            ...
            <div class="contact-row">
                <div class="contact-value-wrap">
                    <i class="fa-solid fa-phone" style="color: #9333ea;"></i>
            ...
            <div class="contact-row">
                <div class="contact-value-wrap">
                    <i class="fa-solid fa-envelope" style="color: #9333ea;"></i>
                    ...
                    ?>
                        <div class="dependant-item-card">
                            <div class="dependant-header">
                                ...
                                <?php else: ?>
                                    <div class="dir-avatar-dependent" style="background: linear-gradient(135deg, #7e22ce 0%, #a855f7 100%);"><?php echo $d_initials; ?></div>
                                <?php endif; ?>
        ...
        <table class="dir-table">
            ...
            <tbody>
                ...
                <tr class="primary-row" 
                    ...
                    <td>
                        <div class="d-flex align-items-center gap-2">
                            <div class="dir-avatar" style="width: 38px; height: 38px; font-size: 0.85rem; border-radius: 8px; background: linear-gradient(135deg, #581c87 0%, #7e22ce 100%);">
                                <?php echo $initials; ?>
                            </div>
                            <div>
                            ...
                            <div class="nested-dependants-inner">
                                <div class="d-flex align-items-center gap-2 mb-2">
                                    <span class="fw-bold text-slate-800 small text-uppercase" style="letter-spacing: 0.04em;">
                                        <i class="fa-solid fa-people-roof text-purple me-1"></i> Family Dependants Registered to <?php echo htmlspecialchars($unit_str); ?> (<?php echo $dep_count; ?>)
                                    </span>
                                </div>
...
function exportDirectoryCSV() {
    ...
    link.setAttribute("download", "Zone_Estate_Directory_" + new Date().toISOString().slice(0, 10) + ".csv");
