<?php
// staff/directory.php - Estate Resident & Contact Directory for Staff and Gate Security
...
// Query all active estate residents, unrestricted by zone
$sql = "SELECT 
    r.id as resident_id,
    r.custom_id,
    r.flat_id,
...
ORDER BY z.name ASC, s.name ASC, b.name ASC, f.number ASC, (CASE WHEN r.type = 'head' THEN 0 ELSE 1 END), r.id ASC";
...
<style>
/* Staff & Security Theme Overrides */
.directory-hero {
    background: linear-gradient(135deg, rgba(15, 118, 110, 0.08) 0%, rgba(16, 185, 129, 0.04) 100%);
...
<div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3 mb-4">
    <div>
        <h2 class="h4 font-bold text-slate-800 m-0">Estate Resident & Gate Directory</h2>
        <p class="text-secondary small mb-0">Search occupants, gate clearance contacts, and verified family dependants across all sectors.</p>
    </div>
    <div class="d-flex flex-wrap align-items-center gap-2">
        ...
        <button class="btn btn-sm text-white" style="background: #0f766e;" id="toggleAllBtn" onclick="toggleAllDependants()">
...
<div class="directory-hero d-flex flex-wrap align-items-center justify-content-between gap-3">
    <div class="d-flex align-items-center gap-3">
        ...
        <div>
            <h4 class="fw-bold text-slate-900 m-0" style="font-size: 1.15rem;">Staff & Security Directory</h4>
            <span class="text-secondary small">Direct intercom, phone, and verification roster.</span>
        </div>
    </div>
    <div class="d-flex flex-wrap align-items-center gap-2">
        ...
        <div class="directory-stat-badge">
            ...
            <span><?php echo number_format($total_households); ?> Units</span>
        </div>
    </div>
</div>
...
<div class="directory-toolbar">
    <div class="d-flex flex-column flex-lg-row align-items-stretch align-items-lg-center justify-content-between gap-3">
        <div class="search-input-wrap">
            ...
            <input type="text" id="dirSearch" placeholder="Search resident name, phone, email, flat number, street..." onkeyup="filterDirectory()">
        </div>
...
<div class="d-flex justify-content-between align-items-center mb-3 text-secondary small px-1">
    <span>Showing <strong id="visibleCount" class="text-slate-900"><?php echo count($households); ?></strong> units</span>
</div>
...
<div id="dirGridView" class="directory-grid">
    ...
    <div class="dir-card" 
        ...
        <div class="dir-card-header">
            ...
            <?php else: ?>
                <div class="dir-avatar" style="background: linear-gradient(135deg, #0f766e 0%, #115e59 100%);"><?php echo $initials; ?></div>
            <?php endif; ?>
            ...
            <div class="dir-info">
                ...
                <div class="d-flex flex-wrap align-items-center gap-1">
                    ...
                    <?php if (!empty($addr['zone_name'])): ?>
                        <span class="badge" style="font-size: 0.68rem; background: rgba(15,118,110,0.1); color: #0f766e;"><?php echo htmlspecialchars($addr['zone_name']); ?></span>
                    <?php endif; ?>
                </div>
        ...
        <div class="dir-contact-body">
            ...
            <div class="contact-row">
                <div class="contact-value-wrap">
                    <i class="fa-solid fa-phone" style="color: #0f766e;"></i>
            ...
            <div class="contact-row">
                <div class="contact-value-wrap">
                    <i class="fa-solid fa-envelope" style="color: #0f766e;"></i>
            ...
            <?php if ($dep_count > 0): ?>
                <button type="button" class="dependants-toggle-bar" onclick="toggleDependants('<?php echo $f_key; ?>', this)">
                    <div class="d-flex align-items-center gap-2">
                        <span class="dep-count-badge" style="background: rgba(15,118,110,0.12); color: #0f766e;">
                    ...
                    ?>
                        <div class="dependant-item-card">
                            <div class="dependant-header">
                                ...
                                <?php else: ?>
                                    <div class="dir-avatar-dependent" style="background: linear-gradient(135deg, #0d9488 0%, #14b8a6 100%);"><?php echo $d_initials; ?></div>
                                <?php endif; ?>

                                <div class="dep-meta-details">
                                    <div class="dep-name"><?php echo htmlspecialchars($d['name']); ?></div>
                                    <span class="dep-rel-tag" style="background: rgba(15,118,110,0.1); color: #0f766e;">
        ...
        <table class="dir-table">
            ...
            <tbody>
                ...
                <tr class="primary-row" 
                    ...
                    <td>
                        <div class="d-flex align-items-center gap-2">
                            <div class="dir-avatar" style="width: 38px; height: 38px; font-size: 0.85rem; border-radius: 8px; background: linear-gradient(135deg, #0f766e 0%, #115e59 100%);">
                                <?php echo $initials; ?>
                            </div>
                            <div>
                    ...
                    <td>
                        <?php if ($dep_count > 0): ?>
                            <span class="dep-count-badge" style="background: rgba(15,118,110,0.12); color: #0f766e; cursor: pointer;" onclick="toggleTableNestedRow('<?php echo $f_key; ?>')">
                            ...
                            <div class="nested-dependants-inner">
                                <div class="d-flex align-items-center gap-2 mb-2">
                                    <span class="fw-bold text-slate-800 small text-uppercase" style="letter-spacing: 0.04em;">
                                        <i class="fa-solid fa-people-roof text-teal me-1" style="color: #0f766e;"></i> Family Dependants Registered to <?php echo htmlspecialchars($unit_str); ?> (<?php echo $dep_count; ?>)
                                    </span>
                                </div>
                                    ...
                                    <tbody>
                                        <?php foreach ($deps as $d): ?>
                                            <tr>
                                                ...
                                                <td><span class="dep-rel-tag" style="background: rgba(15,118,110,0.1); color: #0f766e;"><?php echo htmlspecialchars(ucfirst($d['relationship'] ?? 'Dependent')); ?></span></td>
...
<div id="dirEmptyState" class="text-center py-5" style="display: none;">
    ...
    <p class="text-secondary small mb-3">No occupants or dependants matched your search filters.</p>
...
function exportDirectoryCSV() {
    ...
    link.setAttribute("download", "Staff_Estate_Directory_" + new Date().toISOString().slice(0, 10) + ".csv");
